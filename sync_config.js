...
const CONFIG = {
    spreadsheetId:
        "1FknRUXJq5MrSmKrnQnDdg9cj-te3WAAISKTF0G_4I3k",
...
async function syncCollection(db, config) {
    ...
    const values = await readSheet(config.sheetName);

    console.log(
        `Total Row Sheet : ${values.length}`
    );

    const data = convertToJson(values);
...
async function main() {
    ...
    try {
        ...
        console.log("");
        console.log("ALL CONFIG DATA SYNC SUCCESS.");
        console.log("");
